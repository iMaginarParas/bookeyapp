...
/* 
 * IMPORTANT: Indian Market Configuration
 * 
 * This file is configured for the Indian market with INR pricing.
 * 
 * Required App Store Connect Configuration:
 * - bookey_pro_yearly: ₹999/year subscription 
 * - bookey_credits_50_inr: ₹50 for 7 credits
 * - bookey_credits_100_inr: ₹100 for 14 credits  
 * - bookey_credits_500_inr: ₹500 for 70 credits
 * - bookey_credits_1000_inr: ₹1000 for 140 credits
 * 
 * Make sure these exact product IDs are configured in:
 * 1. App Store Connect (iOS) with INR pricing
 * 2. Google Play Console (Android) with INR pricing
 * 3. RevenueCat dashboard with matching identifiers
 * 
 * If purchases fail with "Product not found", verify:
 * - Product IDs match exactly between code and store
 * - Products are approved and available in India
 * - RevenueCat offerings are configured correctly
 */
...
class _CreditPageState extends State<CreditPage> with TickerProviderStateMixin {
  ...
  final List<CreditPackage> _creditPackages = [
    CreditPackage(
      ...
      credits: 7,
      bonus: '1 scene',
      revenueCatProductId: 'bookey_credits_50_inr',
    ),
    CreditPackage(
      id: 'credits_14_inr',
      name: 'Creator Pack', 
      ...
      bonus: '2 scenes',
      isPopular: true,
      revenueCatProductId: 'bookey_credits_100_inr',
    ),
    CreditPackage(
      id: 'credits_70_inr',
      name: 'Studio Pack',
      ...
      revenueCatProductId: 'bookey_credits_500_inr',
    ),
    CreditPackage(
      id: 'credits_140_inr',
      ...
      revenueCatProductId: 'bookey_credits_1000_inr',
    ),
  ];
  ...
  Future<void> _purchaseCredits(CreditPackage package) async {
    ...
    try {
      ...
      if (success) {
        ...
        throw Exception('Purchase failed - transaction not completed');
      }
    } catch (e) {
      HapticService.heavyImpact();
      ...
      if (e.toString().contains('Product not found')) {
        errorMessage += 'Product not available in your region. Please contact support.';
        print('❌ Product ID ${package.revenueCatProductId} not found in App Store');
        print('💡 Ensure this product ID is configured in App Store Connect for India');
